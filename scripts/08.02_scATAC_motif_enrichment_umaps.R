...
# Nuno/Azizi et al.
# Figure 4
# Plot motif enrichments on umaps for each patient
#
# Workflow for this script is as follows:
# 1. generate motif enrichments
# 2. Plot select motifs
...
for(p in unique(scATAC_ref$patient)){
  ...
  ArchRProjects[[p]] <- addDeviationsMatrix(
    ArchRProj = ArchRProjects[[p]], 
    peakAnnotation = "Motif",
    force = TRUE
  )
  
  plotVarDev <- getVarDeviations(ArchRProjects[[p]], name = "MotifMatrix", plot = TRUE)
  
  plotVarDev
  
  all_motifs<-getFeatures(ArchRProj = ArchRProjects[[p]], useMatrix = "MotifMatrix")
  
  tfs<-c("GATA1","GATA2","CEBPA","HOXA1_","RUNX1","SPI1","MYC_","MYB_","FOS_","JUN_","IRF1","IRF2")
  
  motifs_to_plot<-grep(paste(tfs,collapse = "|"),all_motifs,value = T)
  motifs_to_plot<-grep("deviations",motifs_to_plot,value = T)
  ...
  for(m in motifs_to_plot){
    ...
    pdf(paste0("outputs/scATAC_plots/motif_umaps/",p,"_",gsub("deviations:","",m),"_motif_deviation_umap.pdf"), width = 5, height = 5)
    print(plt)
    dev.off()
  }
}